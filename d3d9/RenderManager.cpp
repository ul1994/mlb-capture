...
bool RenderManager::Draw(RenderInfo &Info)
{
 ...
 //   }
	...
	//if (Type0 == RenderProp || Type1 == RenderProp) {
	//	Options.FullSimulation = true;
	//	printout = true;
	//}
 ...
 //       //MessageBox(NULL, "Rendering prop", "INFO", MB_OK);
 //   }

	//int td = 0;
	//if (Type1 == RenderProp) td = 1;
	//if (Type0 == RenderProp || Type1 == RenderProp) {
 //       const Vec2f Center = Info.ScreenBound.Center();
	//	const Vec2f Dimensions = Info.ScreenBound.Dimensions();
 //       char buffer[256];
 //       sprintf(buffer, "V%d - %d\n(X: %.2f, Y: %.2f - W: %.2f, H: %.2f)",
	//		Info.NumVertices, td, Center.x, Center.y, Dimensions.x, Dimensions.y);
	//	//MessageBox(NULL, buffer, "Ball Found", MB_OK);
	//}

    if(true)
    {
		//// NOTE: partial mesh capture
